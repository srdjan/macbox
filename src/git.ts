...
export const detectRepo = async (repoHint?: string): Promise<RepoInfo> => {
  const root = await mustExec(["git", "rev-parse", "--show-toplevel"], {
    cwd: repoHint,
    label: "git repo root",
  });
  const gitCommonDir = await mustExec(
    ["git", "rev-parse", "--git-common-dir"],
    { cwd: root, label: "git-common-dir" },
  );
  const gitDir = await mustExec(["git", "rev-parse", "--git-dir"], {
    cwd: root,
    label: "git-dir",
  });
  const branch = await mustExec(["git", "rev-parse", "--abbrev-ref", "HEAD"], {
    cwd: root,
    label: "git branch",
  });
  ...
  const abs = (p: string) =>
    p.startsWith("/") ? p : `${root}/${p}`.replaceAll("//", "/");
  return {
    root,
...
export const ensureWorktree = async (
  ...
  const rootGitCommonDir = absFrom(
    root,
    await mustExec(["git", "rev-parse", "--git-common-dir"], {
      cwd: root,
  ...
  try {
    ...
    if (!(err instanceof Deno.errors.NotFound)) {
      ...
      if (
        msg.includes("belongs to a different repository") ||
        msg.includes("is not a directory")
      ) {
...
export const removeWorktree = async (root: string, worktreePath: string) => {
  ...
  await mustExec(["git", "worktree", "remove", "--force", worktreePath], {
    cwd: root,
