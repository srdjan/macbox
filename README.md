...
- `worktreePrefix`: Default worktree name prefix (optional, e.g. `ai-mypreset` becomes `ai-mypreset-1`)
...
Workspaces provide named worktrees with associated session metadata. They are lightweight records that map a human-readable name to a worktree and session.
...
For autonomous iteration over PRDs with quality gates and multi-agent collaboration, use [ralph-cli](https://github.com/srdjan/ralph-cli):
